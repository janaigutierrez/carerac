...
const TRANSPORT_METHODS: { id: string; key: string; icon: LucideIcon; color: string }[] = [
  { id: 'cotxe', key: 'cotxe', icon: Car, color: 'bg-primary-brown' },
  { id: 'public', key: 'public', icon: Bus, color: 'bg-primary-forest' },
  { id: 'peu', key: 'peu', icon: MapPin, color: 'bg-primary-terracotta' },
]

export default function UbicacioSection() {
  const { t } = useLanguage()
    ...
    <section id="ubicacio" ref={ref} className="py-20 bg-primary-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid lg:grid-cols-3 lg:grid-rows-3 gap-6 h-auto lg:h-[600px]">
          {/* Map */}
          <div className={`lg:col-span-2 lg:row-span-3 rounded-xl overflow-hidden shadow-2xl transition-all duration-1000 transform ${inView ? 'opacity-100 scale-100' : 'opacity-0 scale-95'}`} style={{ transitionDelay: '100ms' }}>
            <iframe
          ...
          {/* Transport methods */}
          {TRANSPORT_METHODS.map((method, index) => {
        ...
        <div className="grid lg:grid-cols-3 gap-6 mt-6">
          ...
          <div className={`bg-primary-stone/30 rounded-xl p-6 transition-all duration-1000 transform ${inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`} style={{ transitionDelay: '900ms' }}>
            <h4 className="font-display text-lg font-semibold text-primary-dark mb-3">{t('ubicacio.additionalInfo')}</h4>
